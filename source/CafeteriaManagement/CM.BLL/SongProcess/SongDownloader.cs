...
namespace CafeteriaManagement
{
    public sealed class SongDownloader : IDisposable
    {
        ...
        private static async Task DownloadAudioStreamFrom(VideoInfo videoInfo)
        {
            ...
            var streamManifest = await youtube.Videos.Streams.GetManifestAsync(videoInfo.Url);
            var streamInfo = streamManifest.GetAudioOnlyStreams().GetWithHighestBitrate();
            await youtube.Videos.Streams.DownloadAsync(streamInfo, GetAudioSavePathFrom(videoInfo.Title, false));
        }
